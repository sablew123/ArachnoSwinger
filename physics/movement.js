...
function collideWorld(prevBottom){
  ...
  // adentro de un edificio (ver world/interiors.js): el jugador sigue siendo el MISMO objeto
  // corriendo por el MISMO pipeline de fisica de siempre (updateGroundMovement/updateAirFree ya
  // se ejecutaron este frame con normalidad), pero lo que tiene que resolver la colision es la
  // geometria de la habitacion (paredes/techo/piso propios), no la calle/carros/techos/balcones
  // de la ciudad -- delegamos entera la resolucion a collideRoom (definida en
  // world/interiors.js, se carga despues pero ya esta lista para cuando esto se ejecuta en
  // tiempo real) y cortamos aca, sin tocar nada de lo de abajo
  if(window.insideBuilding){ collideRoom(prevBottom); return; }
