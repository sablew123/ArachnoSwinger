...
(function(){
  ...
  registerEntityType('civil', {
    ...
    onUpdate(e, dt){
      ...
      // mientras viaja jalado por la telaraña (ver performWebPull/updateEntities), el motor ya
      // se encarga de moverlo: aca no hay nada que hacer todavia
      if(e.beingReeled) return;

      // recien llego jalado: pasa a abrazar al jugador (sea cual sea el estado en que estuviera
      // antes -- caminando, conversando, o hasta quietecito y asustado arriba de un techo)
      if(e.justCaught){
        // por carryGroup, no por type==='civil' a secas -- mismo motivo que en performWebPull:
        // si Jary Mane (type 'jaryman') ya esta abrazada y no la contamos aca, un civil nuevo
        // podia terminar heredando el MISMO hugSlot que ella y quedar los dos superpuestos
        const carriedNow = entities.filter(en => (en.def.carryGroup || en.type) === 'civil' && en.carried).length;
        e.state = 'hugging';
        e.carried = true;
        e.vx = 0; e.vy = 0;
        e.dropHoldTimer = 0;
      ...
      // abrazo (ver performWebPull/applyEntityRopePull en el motor). A diferencia del abrazo,
      // el motor no le avisa nada especial a la entidad enganchada -- asi que si no hacemos nada
      // aca, el civil sigue con su plan de caminata de toda la vida mientras lo llevas colgando,
      // sin enterarse de que esta suspendido en el aire de una soga. Eso es justo lo que hacia
      // que, al soltarlo sobre un balcon, siguiera caminando derecho como si nada y se tirara
      // del borde. Por eso llevamos la cuenta nosotros mismos, cuadro a cuadro, de si HAY una
      // telaraña de mano apuntando a esta entidad en este momento.
      ...
      if(towedNow && e.state !== 'towed'){
        ...
        // colgando -- la tension de la soga (applyEntityRopePull) y la gravedad son las que
        // deciden donde termina, no su plan de caminata viejo
        if(e.chatPartner){ e.chatPartner.chatPartner = null; e.chatPartner = null; }
        e.state = 'towed';
        e.vx = 0; e.vy = 0;
      } else if(!towedNow && e.state === 'towed'){
        // la telaraña se soltó (o se cortó): resolver segun donde haya quedado colgando, igual de
        // cuidadoso que al soltar un abrazo (ver dropCivil) -- si quedo parado sobre un techo o
        // balcon, quietecito y asustado ahi (NO seguir caminando y tirarse); si quedo en el aire,
        // que caiga y updateFalling decida al aterrizar; si ya esta en plena calle, sigue su vida
      ...
      if(e.state === 'towed') return; // colgando: nada de IA propia, solo fisica + la soga
  ...
  window.CHUNK_UNLOAD_LISTENERS.push(function(chunkIndex){
    ...
    for(const e of list){
      ...
      // en la que "se resuelve" si te vas del chunk sin volver a bajarlo.
      if(!e.carried && !e.beingReeled && e.state !== 'towed') e.dead = true;
    }
