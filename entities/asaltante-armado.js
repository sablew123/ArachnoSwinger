...
//      civil (tiro de "ataque") tarda ARMED_ATTACK_AIM_WINDUP -- 8 segundos -- apuntando antes de
//      que salga el tiro, y durante esos 8 segundos el civil apuntado grita (mismo radar de borde
...
(function(){
  ...
  const ARMED_SPAWN_CHANCE = 0.03; // ~1 de cada 33 chunks (subido un poquito de 0.025 a pedido, junto con el del comun -- ver ASSAILANT_SPAWN_CHANCE en asaltante.js)
  ...
  const ARMED_ATTACK_AIM_WINDUP = 8; // apuntandole a un civil tarda mucho mas -- 8s (subido de 5 a pedido) -- y en ese rato el civil grita (radar) y queda paralizado (ver updateAI)
  ...
  function updateAI(e, dt){
    ...
    if(Math.abs(dx) <= GUN_RANGE){
      e._huntState = 'aiming';
      e._aimTimer = ARMED_ATTACK_AIM_WINDUP; // apuntandole a un civil: 8s largos, gritando y paralizado (ver HUD_DRAW_LISTENERS y updateAI)
      e._aimKind = 'attack';
      e._aimTarget = target;
      e.facing = dx >= 0 ? 1 : -1;
      e.vx = 0;
  ...
  // ---------- asaltantes armados "huerfanos": el conductor de una moto se convierte en uno de
  // estos al chocar (ver crashMoto/spawnFallingIndependent en asaltante-moto.js), sin pasar por
  // el sorteo por chunk de aca abajo -- por eso nunca reciben _homeChunkIndex y quedan afuera de
  // armedChunkState. Mismo problema y misma solucion que su version desarmada: ver la nota grande
  // junto a orphanFrozen en asaltante.js. Se congelan/reaparecen aparte, en una lista propia sin
  // casillero fijo por chunk (aca tambien podria haber mas de uno por chunk).
  let orphanFrozen = []; // {chunkIndex, x, y, savedState}

  // ---------- guardado/restauracion de estado al congelar/descongelar -- misma idea e igual de
  // motivo que en asaltante.js (ver la nota grande junto a captureFreezeState alla): antes solo
  // se guardaba la posicion, asi que al recargar el chunk el asaltante armado volvia siempre
  // 'walking' con la vida llena, sin importar si habia quedado tumbado a una vida o varado
  // temblando en un techo -- y en ESE ultimo caso, la IA de caceria (updateAI) lo hace caminar
  // libremente detras de un civil sin fijarse si esta parado sobre un techo angosto, asi que se
  // cae de un paso apenas recarga el chunk. Restaurando el estado fisico real esto no pasa.
  ...
  function applyFreezeState(e, saved){
    ...
    if(saved.state === 'lyingDown'){
      ...
      // igual que en asaltante.js: sea que estuviera 'stuck' contra una fachada o 'resting' en el
      // piso, al recargar lo asentamos 'resting' en el piso en la misma posicion -- el array de
      // buildings se regenera al recargar el chunk, asi que no hay garantia de reencontrar el
      // mismo objeto para volver a pegarlo a la fachada exacta
      e.state = 'wrapped';
      e.wrapPhase = 'resting';
      e.wrapTimer = (typeof saved.wrapTimer === 'number') ? saved.wrapTimer : ARMED_WRAP_MELT_TIME;
      e.vx = 0;
    ...
    // 'walking': no hace falta nada mas alla del hp ya asignado
  }
  ...
  window.CHUNK_LOAD_LISTENERS.push(function(chunkIndex, startX, endX, rng){
    ...
    if(e){
      ...
      applyFreezeState(e, st.savedState);
      st.savedState = null;
    }
  });
  ...
  window.CHUNK_UNLOAD_LISTENERS.push(function(chunkIndex){
    for(const [key, st] of armedChunkState.entries()){
      ...
      st.x = e.x; st.y = e.y;
      st.suspended = true;
      st.savedState = captureFreezeState(e);
      e.dead = true;
      st.entity = null;
    }

    // segunda pasada: huerfanos (ver la nota junto a orphanFrozen mas arriba) -- no tienen
    // entrada en armedChunkState, asi que se buscan aparte recorriendo `entities` directamente.
    // Mismo criterio de "parado AHORA en el chunk que se descarga" y mismo chequeo de 911 que la
    // pasada de arriba (window.getAssailant911Chunks, la MISMA señal que usa el asaltante comun).
    for(const e of entities){
      if(e.type !== 'asaltanteArmado' || e.dead) continue;
  ...
  // reaparicion de huerfanos congelados: sin reroll -- si se congelo es porque ya existia, asi
  // que vuelve siempre que su chunk recarga, igual que un asaltante armado "de origen"
  window.CHUNK_LOAD_LISTENERS.push(function(chunkIndex){
    for(let i = orphanFrozen.length-1; i >= 0; i--){
      ...
      const e = spawnEntity('asaltanteArmado', of.x, of.y); // sin _homeChunkIndex a proposito: sigue huerfano
