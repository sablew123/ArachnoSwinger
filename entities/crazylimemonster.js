...
(function(){
  ...
  // Sube el resto del camino, rapido pero SIEMPRE visible, hasta llegar al punto de "un lado del
  // techo" que usa ROOFTOP_BOMBING (rooftopSidePos, misma funcion -- asi al entrar ahi de verdad
  // ya esta pegado al lugar correcto, sin otro salto encima). Se usa en DOS momentos, con el
  // mismo codigo para los dos:
  ...
  //
  // OJO: esto NO es moveToward a velocidad constante (REMOUNT_SPEED como px/s puro) -- con eso,
  // si el boss ya estaba cerca del punto de destino (tipico del caso 1: durante MISSILE se queda
  // dando vueltas cerca del jugador, que es justo quien esta parado en el techo), la distancia a
  // recorrer podia ser tan chica que se cubria en 1-2 cuadros: en la practica se seguia viendo
  // como un salto, aunque tecnicamente "avanzara". Por eso interpola por TIEMPO (mismo smoothstep
  // que EMERGING) con una duracion minima garantizada (REMOUNT_MIN_DURATION) sin importar que tan
  // cerca ya estuviera -- siempre se nota el vuelo.
  const REMOUNT_MIN_DURATION = 0.8; // segundos minimos de subida, aunque la distancia sea corta
  definePhase('REMOUNTING', {
    enter(e){
      e.bombSideSign = 1;
      e.remountTarget = rooftopSidePos(e);
      e.remountFromX = e.x; e.remountFromY = e.y;
      const dist = Math.hypot(e.remountTarget.x - e.x, e.remountTarget.y - e.y);
      e.remountDuration = Math.max(REMOUNT_MIN_DURATION, dist / REMOUNT_SPEED);
    ...
    update(e, dt){
      e.phaseT += dt;
      const t = Math.min(1, e.phaseT / e.remountDuration);
      const s = t*t*(3 - 2*t); // easeInOut suave (smoothstep), mismo criterio que EMERGING
      e.x = e.remountFromX + (e.remountTarget.x - e.remountFromX)*s;
      e.y = e.remountFromY + (e.remountTarget.y - e.remountFromY)*s;
      if(t >= 1){
