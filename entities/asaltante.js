...
(function(){
  ...
  const ASSAILANT_SPAWN_CHANCE = 0.08; // ~1 de cada 12.5 chunks (subido un poquito de 0.07 a pedido)
  ...
  // ---------- asaltantes "huerfanos": nacen sueltos de un duo en moto (ver asaltante-moto.js,
  // crashMoto/convertMotoPassengerToIndependent) en vez de nacer del sorteo por chunk de aca
  // arriba -- por eso nunca reciben un _homeChunkIndex (ese campo solo lo pone el
  // CHUNK_LOAD_LISTENERS de mas abajo, al spawnearlo desde ESTE archivo) y no tienen entrada en
  // assailantChunkState. Sin este segundo registro, la captura pasiva de CHUNK_UNLOAD_LISTENERS
  // (que solo recorria assailantChunkState.entries()) los pasaba por alto: si se descargaba el
  // chunk en el que estaban parados, no se congelaban como cualquier otro asaltante -- ni se les
  // aplicaba el 911/recogida policial -- quedaban corriendo su IA para siempre aunque el chunk ya
  // no estuviera cargado. Este array es la version "sin casillero fijo" de assailantChunkState:
  // en vez de UNA entrada por chunk (que asume un unico origen de aparicion por chunk), acepta
  // cualquier cantidad de huerfanos en un mismo chunk (dos duos distintos podrian resolverse en
  // la misma zona). Al recargarse el chunk, reaparecen en su misma posicion como un 'asaltante'
  // de a pie normal, ya limpio (sin heridas ni estado especial previo) -- exactamente el mismo
  // trato que ya recibe hoy un asaltante "de origen" al descongelarse (ese tambien pierde
  // hp/estado especial al recongelarse via spawnEntity+onSpawn, asi que esto no es una perdida de
  // fidelidad nueva, es la MISMA regla aplicada parejo).
  let orphanFrozen = []; // {chunkIndex, x, y, savedState}

  // ---------- guardado/restauracion de estado al congelar/descongelar ----------
  // ANTES, congelar un asaltante solo guardaba su posicion (st.x/st.y): al recargar el chunk,
  // spawnEntity+onSpawn lo hacia renacer SIEMPRE en 'walking' con la vida llena, sin importar en
  // que estado hubiera quedado. Eso rompia dos cosas bien distintas:
  //   1) Si lo dejaste ACOSTADO a una vida (downedForGood) o TEMBLANDO varado en un techo
  //      (strandedOnRoof) SIN llamar al 911 antes de irte, volvia sano y "caminando" de nuevo --
  //      perdia por completo el progreso de haberlo casi derrotado.
  //   2) Peor todavia con 'strandedOnRoof': al renacer en 'walking', la IA de caceria (updateHunting)
  //      lo hace caminar libremente en busca de un civil SIN importarle que este parado sobre un
  //      techo/balcon angosto -- coincidencia servida, en un paso se cae del borde y se pega el
  //      porrazo (o algo peor) contra la calle. Si en cambio recupera 'strandedOnRoof' de una, se
  //      queda quieto temblando en el lugar exacto donde estaba, como si nunca te hubieras ido.
  // Guardamos lo minimo que hace falta para reconstruir el estado fisico relevante: vida, el
  // estado en si, y los campos propios de cada uno (downedForGood/lieTimer, trembleCycle,
  // wrapPhase/wrapTimer). NO guardamos _huntState/_fleeTimer (si estaba huyendo o a mitad de un
  // ataque): al recargar retoma la caceria de cero (_huntState arranca en 'hunting' via onSpawn),
  // que es un progreso mucho mas menor de perder que quedar caido/varado y no saberlo.
  function captureFreezeState(e){
    return {
      hp: e.hp,
      state: e.state,
      vx: e.vx, vy: e.vy,
      facing: e.facing,
      downedForGood: e.downedForGood,
      lieTimer: e.lieTimer,
      wrapPhase: e.wrapPhase,
      wrapTimer: e.wrapTimer,
      trembleCycle: e.trembleCycle
    };
  }

  function applyFreezeState(e, saved){
    if(!saved) return; // resguardo: sin estado guardado (no deberia pasar), se queda tal cual nacio
    e.hp = saved.hp;
    if(typeof saved.facing === 'number') e.facing = saved.facing;

    if(saved.state === 'lyingDown'){
      enterLyingDown(e, saved.downedForGood);
      if(!saved.downedForGood && typeof saved.lieTimer === 'number') e.lieTimer = saved.lieTimer;
    } else if(saved.state === 'strandedOnRoof'){
      enterStranded(e);
      if(typeof saved.trembleCycle === 'number') e.trembleCycle = saved.trembleCycle;
    } else if(saved.state === 'wrapped'){
      // sea que estuviera 'stuck' (pegado a una fachada) o 'resting' (tirado en el piso), al
      // recargar lo asentamos en 'resting' en el piso, en la MISMA posicion (st.x/st.y) -- no
      // intentamos volver a pegarlo a una fachada porque el array de buildings se regenera al
      // recargar el chunk y no hay garantia de encontrar el mismo objeto; el timer de derretido
      // sigue corriendo igual, asi que el efecto practico (cuanto le queda envuelto) se conserva
      e.state = 'wrapped';
      e.wrapPhase = 'resting';
      e.wrapTimer = (typeof saved.wrapTimer === 'number') ? saved.wrapTimer : ASSAILANT_WRAP_MELT_TIME;
      e.vx = 0;
    } else if(saved.state === 'falling'){
      // lo agarro el chunk unload en pleno vuelo (p.ej. recien eyectado de una moto que choco):
      // retoma la caida con el mismo impulso en vez de arrancar de cero parado en el piso
      e.state = 'falling';
      e.vx = saved.vx || 0;
      e.vy = saved.vy || 0;
      e.grounded = false;
    }
    // 'walking' (o cualquier estado no cubierto arriba): no hace falta tocar nada mas alla del hp
    // ya asignado -- nace caminando via onSpawn, que es exactamente lo que corresponde
  }
  ...
  window.CHUNK_LOAD_LISTENERS.push(function(chunkIndex, startX, endX, rng){
    ...
    if(e){
      ...
      applyFreezeState(e, st.savedState); // recupera vida/estado (asustado, a una vida, etc.) si venia congelado con alguno
      st.savedState = null;
    }
  });
  ...
  window.CHUNK_UNLOAD_LISTENERS.push(function(chunkIndex){
    ...
    for(const [key, st] of assailantChunkState.entries()){
      ...
      st.x = e.x; st.y = e.y;
      st.suspended = true;
      st.savedState = captureFreezeState(e); // ver la nota grande junto a captureFreezeState
      e.dead = true;    // solo lo saca de `entities` -- NO dispara onDeath (eso es solo por hp<=0), asi que no cuenta como derrotado
      st.entity = null;
    }

    // segunda pasada: los huerfanos (ver la nota grande junto a orphanFrozen mas arriba) no
    // tienen entrada en assailantChunkState, asi que hay que buscarlos aparte, recorriendo
    // `entities` directamente. Mismo criterio de "esta parado AHORA en el chunk que se descarga"
    // y mismo chequeo de 911/lyingDown-wrapped-strandedOnRoof que la pasada de arriba.
    for(const e of entities){
      if(e.type !== 'asaltante' || e.dead) continue;
      if(typeof e._homeChunkIndex === 'number') continue; // tiene su propio casillero: ya lo cubrio el loop de arriba
      if(e.carried || e.beingReeled || e.state === 'towed') continue;
      if(Math.floor(e.x / CHUNK_WIDTH) !== chunkIndex) continue;
      ...
      orphanFrozen.push({chunkIndex, x: e.x, y: e.y, savedState: captureFreezeState(e)});
      e.dead = true; // solo lo saca de `entities`, igual que a un asaltante "de origen" (ver arriba)
    }
  });

  // reaparicion de huerfanos congelados: SIN reroll ni chance de aparicion (a diferencia del
  // sorteo normal de mas arriba) -- si se congelo aca es porque ya existia, asi que vuelve
  // siempre, tal cual como un asaltante "de origen" se descongela siempre que su chunk recarga
  window.CHUNK_LOAD_LISTENERS.push(function(chunkIndex){
    for(let i = orphanFrozen.length-1; i >= 0; i--){
      const of = orphanFrozen[i];
      if(of.chunkIndex !== chunkIndex) continue;
      orphanFrozen.splice(i, 1);
      const e = spawnEntity('asaltante', of.x, of.y); // sin _homeChunkIndex a proposito: sigue siendo huerfano, listo para congelarse de nuevo la proxima si hace falta
      if(e) applyFreezeState(e, of.savedState);
    }
  });
