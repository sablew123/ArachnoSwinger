...
(function(){
  ...
  const ASSAILANT_SPAWN_CHANCE = 0.08; // ~1 de cada 12-13 chunks (subido un poco de 0.05)
  ...
  function physicsStep(e, dt){
    ...
    void wasGrounded; // (sin daño de caida para el asaltante: a proposito mas simple que un civil, no hace falta acumular impacto)
  }
  ...
  // salvo que aca NO hay daño de caida que amortiguar (el asaltante ya de por si no tiene daño de
  // caida -- ver la nota en physicsStep mas arriba, "a proposito mas simple que un civil")
  ...
  function onDeath(e){
    ...
    const st = assailantChunkState.get(e._homeChunkIndex);
    if(st) st.defeated = true; // esta era su unica vida: este chunk se queda sin asaltante para siempre
  ...
  // propia POR CHUNK (no una unica global): cada chunk que le toco en suerte tener asaltante lo
  // conserva -- vivo, congelado mientras el chunk esta descargado, o definitivamente vencido --
  // durante toda la partida ----------
  const assailantChunkState = new Map(); // chunkIndex -> {hasOne, defeated, suspended, entity, x, y}
  ...
  window.CHUNK_LOAD_LISTENERS.push(function(chunkIndex, startX, endX, rng){
    ...
    if(!st){
      // primera vez que se visita este chunk: se tira la moneda UNA sola vez con el rng propio de
      // este chunk (chunkEntityRng en el motor: mismo chunk = misma secuencia siempre) y la
      // decision queda grabada para siempre -- asi un chunk no "gana" un asaltante nuevo cada vez
      // que se recarga
      const roll = rng();
      if(roll < ASSAILANT_SPAWN_CHANCE){
        const x = startX + 80 + rng()*(endX - startX - 160);
        st = {hasOne: true, defeated: false, suspended: false, entity: null, x, y: STREET_Y - ASSAILANT_HIT_RADIUS};
      } else {
        st = {hasOne: false};
      }
    ...
    if(!st.hasOne || st.defeated) return;
  ...
  window.CHUNK_UNLOAD_LISTENERS.push(function(chunkIndex){
    ...
    // de nacimiento.
    for(const st of assailantChunkState.values()){
      if(!st.hasOne || st.defeated || !st.entity || st.entity.dead) continue;
      const e = st.entity;
      ...
      // si lo dejas ACOSTADO (a una vida, tumbado -- ver enterLyingDown en onDamage) o ENVUELTO en
      // una telaraña suelta (state 'wrapped') justo cuando te vas del chunk, Y ADEMAS ya llamaste al
      // 911 en algun momento desde este chunk o alguno de sus 2 vecinos (los "3 chunks cercanos" --
      ...
      // policia -- ya avisada -- lo encontro asi de indefenso y se lo llevo mientras no mirabas: este
      // chunk se queda sin asaltante para siempre, IGUAL que si lo hubieras terminado a golpes
      // (st.defeated). SIN la llamada previa, no hay a quien avisarle: se congela tal cual y vuelve
      // a funcionar al recargar, igual que dejarlo deambulando/persiguiendo/huyendo con normalidad
      ...
      if((e.state === 'lyingDown' || e.state === 'wrapped') && chunkHasNearby911Call){
        st.defeated = true;
        e.dead = true;
        st.entity = null;
  ...
  registerEntityType('asaltoBus', {
    ...
    onUpdate(e, dt){
      ...
      for(const other of entities){
        if(other.type !== 'asaltante' || other.dead) continue;
