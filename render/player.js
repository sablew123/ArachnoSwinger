...
//
// SKIN: si spiderswing.html cargo una skin pintada desde el menu (variable global playerSkin,
// ver loadPlayerSkin ahi), drawPlayer estampa el sprite de cada segmento (color base + dibujo a
// pincel) ENCIMA del trazo liso de siempre, ya rotado/escalado a la posicion y angulo reales de
// ESTE frame -- no reemplaza nada del dibujo original ni de la fisica/hitbox, solo se agrega por
// arriba. Si no hay playerSkin (o a un segmento puntual le falta sprite), ese segmento
// simplemente se ve como siempre.

// fraccion del canvas 320x320 del editor de skins (ver buildSegmentPath en index.html) que
// ocupa la silueta real de cada forma -- hace falta para escalar el sprite guardado al
// largo/grosor EXACTO de cada segmento (el resto del canvas, alrededor de la silueta, es
// transparente)
const SKIN_SHAPE_FRAC = {
  circle:  {w: 0.68, h: 0.68}, // diametro = 2 * radio(0.34) del editor
  capsule: {w: 0.82, h: 0.32}
};
const SKIN_LIMB_THICKNESS = 4.5; // grosor con el que se estampa torso/brazos/piernas -- apenas mas ancho que el trazo liso de base (3px); un numero mas alto queda "gordo" porque los segmentos son bien cortos (el torso mide apenas 11px de largo, por ejemplo)

// estampa el sprite ya compuesto de un segmento tipo "capsula" (torso/brazo/pierna) a lo largo
// del segmento local start->end, rotado y escalado para quedar en la posicion/angulo correctos
// de este frame -- sea cual sea la pose actual (caminando, colgado, atacando, etc.)
function stampSkinLimb(ctx, sprite, start, end, thickness){
  const frac = SKIN_SHAPE_FRAC.capsule;
  const dx = end.x-start.x, dy = end.y-start.y;
  const length = Math.hypot(dx,dy) || 0.001;
  const angle = Math.atan2(dy,dx);
  const boxW = length / frac.w, boxH = thickness / frac.h;
  ctx.save();
  ctx.translate(start.x, start.y);
  ctx.rotate(angle);
  ctx.drawImage(sprite, length/2 - boxW/2, -boxH/2, boxW, boxH);
  ctx.restore();
}

// estampa el sprite de la cabeza, centrado en "center" con el radio pedido (mismo radio que ya
// usa el circulo liso de la cabeza, asi queda del mismo tamaño de siempre)
function stampSkinHead(ctx, sprite, center, radius){
  const frac = SKIN_SHAPE_FRAC.circle;
  const boxSize = (radius*2) / frac.w;
  ctx.save();
  ctx.translate(center.x - boxSize/2, center.y - boxSize/2);
  ctx.drawImage(sprite, 0, 0, boxSize, boxSize);
  ctx.restore();
}
...
function drawPlayer(ctx){
  ...
  if(typeof playerSkin !== 'undefined' && playerSkin && playerSkin.legLeft) stampSkinLimb(ctx, playerSkin.legLeft.sprite, pose.hip, pose.leftFoot, SKIN_LIMB_THICKNESS);
  ctx.beginPath(); ctx.moveTo(pose.hip.x,pose.hip.y); ctx.lineTo(pose.rightFoot.x,pose.rightFoot.y); ctx.stroke();
  if(typeof playerSkin !== 'undefined' && playerSkin && playerSkin.legRight) stampSkinLimb(ctx, playerSkin.legRight.sprite, pose.hip, pose.rightFoot, SKIN_LIMB_THICKNESS);

  // torso
  ctx.beginPath(); ctx.moveTo(0,-7); ctx.lineTo(0,4); ctx.stroke();
  if(typeof playerSkin !== 'undefined' && playerSkin && playerSkin.torso) stampSkinLimb(ctx, playerSkin.torso.sprite, {x:0,y:-7}, {x:0,y:4}, SKIN_LIMB_THICKNESS);

  ctx.beginPath(); ctx.moveTo(pose.leftShoulder.x,pose.leftShoulder.y);  ctx.lineTo(pose.leftHand.x,pose.leftHand.y);  ctx.stroke();
  if(typeof playerSkin !== 'undefined' && playerSkin && playerSkin.armLeft) stampSkinLimb(ctx, playerSkin.armLeft.sprite, pose.leftShoulder, pose.leftHand, SKIN_LIMB_THICKNESS);
  ...
  if(typeof playerSkin !== 'undefined' && playerSkin && playerSkin.armRight) stampSkinLimb(ctx, playerSkin.armRight.sprite, pose.rightShoulder, pose.rightHand, SKIN_LIMB_THICKNESS);

  // cabeza
  ctx.beginPath(); ctx.arc(0,-11,5,0,Math.PI*2); ctx.fill();
  if(typeof playerSkin !== 'undefined' && playerSkin && playerSkin.head) stampSkinHead(ctx, playerSkin.head.sprite, {x:0,y:-11}, 5);
